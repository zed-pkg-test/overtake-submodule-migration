# overtake-submodule-migration

    Independent **interoperability** harness in `zed-pkg-test` for `zed-pkg`.

    **Readiness:** `ready`  
    **Primary dependency strategy:** `git-submodule`  
    ...
    **Live infrastructure:** None for deterministic pull-request checks.
    ...
    1. Verify conversion of nested .gitmodules into pinned .zpkg.toml and .zpkg.lock dependencies across the supported happy-path states and canonical fixtures.
2. Verify conversion of nested .gitmodules into pinned .zpkg.toml and .zpkg.lock dependencies under retries, interruption, concurrency, offline operation, or partial failure.
3. Verify conversion of nested .gitmodules into pinned .zpkg.toml and .zpkg.lock dependencies preserves authorization, idempotency, integrity, observability, and actionable failure classification.
